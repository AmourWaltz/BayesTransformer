...
gpu_num = len(args.devices)
devices = list(range(gpu_num))
...
if args.optimizer != 'asgd':
    if args.scheduler == 'cosine':
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,
                                                               args.max_decay_step - args.warmup_step,
                                                               eta_min=args.lr_min)
...
def train():
    ...
    mems = tuple()
    batch, i = 0, 0
    d_model = torch.nn.DataParallel(model.cuda(), device_ids=devices)
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        ##### Forward
        ret = d_model(data, target, *mems, return_h=True)
