...
class Corpus(object):
    ...
    def __init__(self, args, data_path, valid_batch=10, test_batch=1, use_voc=True):
        self.use_voc = use_voc
        self.dictionary = Dictionary()
        if args.dataset == 'ptb' and args.sentence_level is False:
            self.voc = Vocabulary(os.path.join(data_path, 'voc.txt'), use_num=False, data_version=args.dataset)
            self.train_data = self.tokenize(os.path.join(data_path, 'train.txt'))
            self.valid_data = self.tokenize(os.path.join(data_path, 'valid.txt'))
            self.test_data = self.tokenize(os.path.join(data_path, 'test.txt'))
            self.valid_loader = None
            self.test_loader = None
            pass
        elif args.dataset == 'ptb' and args.sentence_level is True:
            self.voc = Vocabulary(os.path.join(data_path, 'voc.txt'), use_num=False, data_version=args.dataset)
            self.train_data = self.tokenize(os.path.join(data_path, 'train.txt'))
            self.valid_data = TextDataset(os.path.join(data_path, 'valid.txt'), self.voc)
            self.test_data = TextDataset(os.path.join(data_path, 'test.txt'), self.voc)
            ...
            self.test_loader = data.DataLoader(self.test_data, batch_size=test_batch,
                                               shuffle=False, num_workers=0, collate_fn=collate_fn, drop_last=False)
            pass
        elif args.dataset == 'swbd' and args.sentence_level is True:
            self.voc = Vocabulary(os.path.join(data_path, 'words.txt'), use_num=False, data_version=args.dataset)
            self.train_data = self.tokenize(os.path.join(data_path, 'fisher.txt'))
            self.valid_data = TextDataset(os.path.join(data_path, 'dev.txt'), self.voc)
            self.test_data = TextDataset(os.path.join(data_path, 'swbd.txt'), self.voc)
            self.valid_loader = data.DataLoader(self.valid_data, batch_size=valid_batch,
                                                shuffle=False, num_workers=0, collate_fn=collate_fn, drop_last=False)
            self.test_loader = data.DataLoader(self.test_data, batch_size=test_batch,
                                               shuffle=False, num_workers=0, collate_fn=collate_fn, drop_last=False)
            pass
        elif args.dataset == 'swbd' and args.sentence_level is False:
            self.voc = Vocabulary(os.path.join(data_path, 'words.txt'), use_num=False, data_version=args.dataset)
            self.train_data = self.tokenize(os.path.join(data_path, 'fisher.txt'))
            self.valid_data = self.tokenize(os.path.join(data_path, 'dev.txt'))
            self.test_data = self.tokenize(os.path.join(data_path, 'swbd.txt'))
            self.valid_loader = None
            self.test_loader = None
            pass
        pass
